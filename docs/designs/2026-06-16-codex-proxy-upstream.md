...
- **Critical, but a two-line fix (not a new flag).** "Is this the Codex backend?" is computed at **`agent/chat_completion_helpers.py:574`** (`is_codex_backend = agent.provider == "openai-codex" or (hostname == "chatgpt.com" and …)`) — pointed at localhost with provider `codex-proxy`, both disjuncts are false → `max_output_tokens` is wrongly sent (rejected by the Codex backend, `codex.py:239`) and session headers skipped (`codex.py:220`). That expression **already special-cases provider names** (`is_xai_responses = agent.provider in {"xai","xai-oauth"} …` at line 581), so the idiomatic fix is to add `"codex-proxy"` to the `openai-codex` set there, and likewise to the `api_mode` disjunction at `agent_init.py:295`. No `force_codex_backend` flag, no new dataclass fields — ~2 lines, mirroring how xAI is handled. (`api_mode` is also covered by the overlay's `transport="codex_responses"` → `TRANSPORT_TO_API_MODE` mapping; the `agent_init.py` edit is belt-and-suspenders for the `provider_name`-set path.)
...
1. **Gate 2 — VERIFIED (2026-06-16 spike).** The load-bearing assumption holds. A throwaway proxy (real token + the three Cloudflare headers → `chatgpt.com/backend-api/codex`) plus an isolated `CODEX_HOME` config with a custom `model_provider` (`wire_api="responses"`, dummy `env_key=SPIKE_KEY`, `base_url=http://127.0.0.1:8766/v1`) was driven by `codex exec`. Result: `codex` used the custom provider **without any ChatGPT login**, sent a 43 KB Responses request through the proxy, the backend returned **200**, and the CLI printed the expected completion. So the codex CLI/plugin leg is real, not hypothetical — the proxy genuinely fronts the external CLI on the free subscription, which is its advantage over the symlink. Residual: confirmed only from this residential Mac IP (Gate 3 still environmental for VPS).
