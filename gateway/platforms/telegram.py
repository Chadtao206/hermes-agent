...
class _TelegramParseModeStr(str):
    """String parse-mode value whose repr preserves the symbolic enum name.

    Some tests run with a minimal Telegram SDK mock where ParseMode.MARKDOWN_V2
    is the plain string "MarkdownV2".  Production SDKs may expose a string-like
    enum.  Returning a str subclass keeps Bot API compatibility while preserving
    the symbolic repr those tests assert.
    """

    def __new__(cls, value: str, symbolic_name: str):
        obj = str.__new__(cls, value)
        obj._symbolic_name = symbolic_name
        return obj
...
def _markdown_v2_parse_mode():
    mode = getattr(ParseMode, "MARKDOWN_V2", "MarkdownV2")
    if type(mode) is str:
        return _TelegramParseModeStr(mode, "ParseMode.MARKDOWN_V2")
    return mode
                    ...
                    try:
                        ...
                        try:
                            msg = await self._bot.send_message(
                                chat_id=int(chat_id),
                                text=chunk,
                                parse_mode=_markdown_v2_parse_mode(),
                                reply_to_message_id=reply_to_id,
                                **thread_kwargs,
            ...
            for use_markdown in (True, False) if finalize else (False,):
                try:
                    ...
                    sent_msg = await self._bot.send_message(
                        chat_id=int(chat_id),
                        text=text,
                        parse_mode=_markdown_v2_parse_mode() if use_markdown else None,
                        reply_to_message_id=reply_to_id,
                        **thread_kwargs,
                        **self._link_preview_kwargs(),
                        **self._notification_kwargs(metadata),
