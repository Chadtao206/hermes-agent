...
def build_gateway_parser(
    ...
    # codex-proxy: launchd/systemd service management (fork feature; handler in
    # hermes_cli/proxy/cli.py routes proxy_command install/uninstall).
    proxy_install = proxy_subparsers.add_parser(
        "install", help="Install the codex proxy as a launchd/systemd background service"
    )
    proxy_install.add_argument(
        "--port", type=int, default=None, help="Bind port (default: 8645)"
    )
    proxy_subparsers.add_parser(
        "uninstall", help="Uninstall the codex proxy background service"
    )
