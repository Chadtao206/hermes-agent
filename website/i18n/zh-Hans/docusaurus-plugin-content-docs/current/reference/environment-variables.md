...
| `HERMES_KANBAN_HOME` | 覆盖锚定 kanban 工作区、worker 日志和旧版 SQLite 看板文件的共享 Hermes 根目录。回退到 `get_default_hermes_root()`（任意活动 profile 的父目录）。适用于测试和非常规部署 |
...
| `HERMES_KANBAN_DB` | 旧版/仅 SQLite 的覆盖项，用于直接固定 kanban 数据库文件路径。当配置 `kanban.backend: postgres` 时，不要把它当作权威看板选择器；看板作用域使用 `HERMES_KANBAN_BOARD`，Postgres 连接使用 `HERMES_KANBAN_PG_DSN` / `kanban.postgres.dsn`。 |
| `HERMES_KANBAN_PG_DSN` | `kanban.backend: postgres` 的 Postgres DSN 覆盖项。优先于 `config.yaml` 中的 `kanban.postgres.dsn`；该值应作为秘密处理。 |
