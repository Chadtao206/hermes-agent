...
Hermes Kanban 是一个持久化任务看板，在所有 Hermes 配置文件之间共享，允许多个具名 agent 协作完成工作，而无需脆弱的进程内子 agent 集群。当前/默认部署使用 Postgres（`kanban.backend: postgres`）；每个任务和交接都是配置的 Kanban store 中的持久化记录；每个 worker 都是拥有独立身份的完整 OS 进程。

### 两个操作界面：模型通过工具交互，你通过 CLI 交互

看板有两个入口，均由同一个配置的 Kanban store 支撑：
...
看板让你将不相关的工作流分离到独立的队列中 —— 每个项目、仓库或领域一个。新安装只有一个名为 `default` 的看板。只需要一个工作流的用户无需了解看板；该功能是可选启用的。

每个看板的隔离是绝对的：

- Postgres 中的独立看板命名空间（同样通过 `--board` / `HERMES_KANBAN_BOARD` 作用域控制）。旧版 SQLite 部署仍以每看板 DB 文件隔离，但新的/默认部署使用 Postgres。
...
将 `hermes kanban daemon` 作为单独进程运行已**弃用**；请使用 gateway。如果你确实无法运行 gateway（无头主机策略禁止长期运行的服务等），`--force` 逃生舱口在一个发布周期内保持旧的独立守护进程可用，但同时运行 gateway 内嵌调度器和针对同一 Kanban store 的独立守护进程会导致认领竞争，不受支持。
...
1. **后端可移植性。** 终端工具指向远程后端（Docker / Modal / Singularity / SSH）的 worker 会在容器*内部*运行 `hermes kanban complete`，而容器中可能没有安装 `hermes`，也没有主机 Kanban 凭据/配置。kanban 工具在 agent 自己的 Python 进程中运行，无论终端后端如何，始终能访问配置的 Kanban store。
...
```bash
hermes kanban init      # 一次性：初始化配置的 Kanban store
...
每个处理器都是一个薄封装 —— 插件约 700 行 Python（路由器 + WebSocket 追踪 + 批量处理器 + 配置读取器），不添加任何新的业务逻辑。它在每次读写时打开配置的 Kanban store，因此无论用户是先打开仪表盘、直接访问 REST API，还是运行 `hermes kanban init`，全新安装都能正常工作。
...
Kanban store 中的任务是有意与配置文件无关的（这是协调原语）。如果你用 `hermes -p <profile> dashboard` 打开仪表盘，看板仍然显示使用同一配置 store 的任何其他配置文件创建的任务。同一用户拥有所有配置文件，但如果多个角色共存，这一点值得了解。
...
```
hermes kanban init                                     # 初始化配置的 Kanban store
...
Gateway 通常在 agent 仍在思考时将斜杠命令和用户消息排队 —— 这就是防止你在第一轮还在进行时意外启动第二轮的机制。**`/kanban` 被明确豁免于此保护。** 看板存在于配置的 Kanban store 中，而不是运行中 agent 的状态中，因此读取（`list`、`show`、`context`、`tail`、`watch`、`stats`、`runs`）和写入（`comment`、`unblock`、`block`、`assign`、`archive`、`create`、`link` 等）都会立即执行，即使在轮次进行中。
...
Kanban 的权威 store 可以是 Postgres，但默认调度器执行模型仍在运行 gateway 的本机启动 worker。多台主机可以查看同一个 Postgres 看板，但针对同一看板运行多个独立调度器需要明确的运维协调；除非你设计了多主机 worker 拓扑，否则 worker 崩溃检测和 PID 处理仍假设本地主机。
