...
| `HERMES_KANBAN_HOME` | Override the shared Hermes root that anchors kanban workspaces, worker logs, and legacy SQLite board files. Falls back to `get_default_hermes_root()` (the parent of any active profile). Useful for tests and unusual deployments |
...
| `HERMES_KANBAN_DB` | Legacy/SQLite-only override to pin the kanban database file path directly. Do not use it as the authoritative board selector when `kanban.backend: postgres` is configured; use `HERMES_KANBAN_BOARD` for board scoping and `HERMES_KANBAN_PG_DSN` / `kanban.postgres.dsn` for the Postgres connection. |
| `HERMES_KANBAN_PG_DSN` | Postgres DSN override for `kanban.backend: postgres`. Takes precedence over `kanban.postgres.dsn` in `config.yaml`; keep the value secret. |
