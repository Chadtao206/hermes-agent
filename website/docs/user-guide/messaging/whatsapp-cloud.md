...
- **Images** — auto-downloaded to the local cache, attached to the model's user message via the gateway's standard vision-routing layer.  Models with native vision (Claude, GPT-4o, Gemini 2.5 Pro, etc.) see the image directly; non-vision models get a `vision_analyze` description.
- **Voice notes** — auto-downloaded as `.ogg`, transcribed via your configured STT provider (local faster-whisper, Groq Whisper, OpenAI Whisper via Nous subscription, etc.), then handed to the agent as text.
- **Documents** — auto-downloaded.  Small text-readable types (`.txt`, `.md`, `.json`, `.py`, `.csv`, etc.) up to 100KB have their content inlined into the user message so the agent can read it without a tool call.  Larger files are cached locally for the agent's other tools to access.
- **Button replies / interactive list replies** — surfaced as text events so the agent can reason about the user's choice.
- **Reply context** — when the user replies to a previous bot message, `MessageEvent.reply_to_message_id` gets populated and the agent's prompt builder includes the original message as context.

### Outbound

- **Text** — markdown is automatically converted to WhatsApp's flavored syntax (`**bold**` → `*bold*`, `~~strike~~` → `~strike~`, headers → bold, `[link](url)` → `link (url)`).  Long messages split at 4096 chars per chunk.
- **Images** — agent's `image_generate` tool's URL output is delivered via Meta's link-mode (one Graph API call).  Local image files use upload-then-reference (two calls).
- **Voice messages** — agent's `text_to_speech` MP3 output is converted to opus via ffmpeg before sending, producing the native WhatsApp voice-note bubble.  Without ffmpeg, falls back to MP3 audio attachment (still plays inline, but shows as a file rather than waveform — see "Voice messages" below for details).
- **Video / documents** — similar link-or-upload paths, both supported.

### Voice messages

WhatsApp distinguishes between a "voice note" (the green waveform bubble) and a generic audio file attachment.  The difference is purely codec: voice notes are `audio/ogg` containers with the `opus` codec.

Hermes TTS produces MP3.  Two paths:

- **With ffmpeg on PATH** (recommended) — outbound TTS goes through `ffmpeg` and arrives as a proper voice note.  Install:
...
Meta only allows **free-form messages** within a 24-hour window after the user's last inbound message.  Outside that window, the only thing the API accepts is a pre-approved **message template**.

**What this means in practice:**

- Reactive chat (user DMs → bot replies within 24h → user replies → ...) works forever.
- **Cron jobs that deliver to WhatsApp** after a gap > 24h will fail with Graph error code `131047` ("Re-engagement message").
- **`delegate_task` async results** that take longer than 24h fail the same way.
- **Webhook subscribers** that route external events to WhatsApp fail when the user hasn't DM'd the bot recently.

Template support (the workaround) is not yet implemented in Hermes.  If you need it, please [open an issue](https://github.com/NousResearch/hermes-agent/issues) — it's planned but deprioritized while we wait for demand signal.
