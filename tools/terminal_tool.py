...
Background: Set background=true to get a session_id. Two patterns:
  ...
  (2) Long-running tasks with notify_on_complete=true — you can keep working on other things and the system auto-notifies you when the task finishes. Great for test suites, builds, deployments, CI/check watchers, or anything that takes more than a minute.
