...
Foreground (default): Commands return INSTANTLY when done, even if the timeout is high. Set timeout=300 for long builds/scripts — you'll still get the result in seconds if it's fast. Prefer foreground for short commands. Do NOT run long foreground polling loops for CI/check/deploy status; use one-shot status checks, background=true with notify_on_complete, or a cron/no-agent watchdog instead.
Background: Set background=true to get a session_id. Almost always pair bounded background tasks with notify_on_complete=true — bg without notify runs SILENTLY and you have no way to learn it finished short of calling process(action='poll') yourself. Two legitimate uses:
  (1) Long-lived processes that never exit (servers, watchers, daemons) — silent is correct, there's no exit to notify on.
  (2) Long-running bounded tasks (tests, builds, deploys, CI pollers, batch jobs) — MUST set notify_on_complete=true. You can keep working and the system auto-notifies you when it finishes; without it you'll either forget to poll or sit blocked waiting for the user to surface the result.
