...
MEMORY_GUIDANCE = (
    "You have persistent memory. Save only compact, declarative facts that will still matter later "
    "(preferences, environment facts, conventions, recurring corrections, tool quirks). "
    "Do not save temporary task state or stale artifacts; use session_search for past work and skills for reusable procedures."
)

SESSION_SEARCH_GUIDANCE = (
    "If relevant prior conversation context may exist, use session_search before asking the user to repeat it."
)

SKILLS_GUIDANCE = (
    "After non-trivial work or a durable correction, capture reusable workflows as skills. "
    "If a loaded skill is stale or wrong, patch it immediately."
)

KANBAN_GUIDANCE = (
    "# Kanban task execution protocol\n"
    "You have been assigned ONE task from "
    "the shared Kanban board. Your task id is in "
    "`$HERMES_KANBAN_TASK`; your workspace is `$HERMES_KANBAN_WORKSPACE`. "
    "The `kanban_*` tools in your schema are your primary coordination surface — "
    "they write directly to the configured shared store (Postgres on this "
    "machine) and work regardless of terminal "
    ...
    "during long subprocesses (training, encoding, crawling); skip it for "
    "short tasks. **If your task may exceed 1 hour, you MUST heartbeat at "
    "least hourly** — the dispatcher reclaims tasks with no heartbeat in the "
    "last hour (past `kanban.dispatch_stale_timeout_seconds`, default 4h), "
    "re-queuing them as `ready` without penalty, but you lose this run's "
    "progress.\n"
    ...
    "workers read both. Never put secrets / tokens / raw PII in either — run "
    "rows are durable forever. Exception: if your output is a code change "
    "needing human review (most coding tasks), put the metadata in a "
    ...
    "- Do not directly inspect, copy, replace, repair, VACUUM, checkpoint, or "
    "otherwise mutate the underlying Kanban store from a worker. On current "
    "Postgres-backed installs, `~/.hermes/kanban.db` / `$HERMES_KANBAN_DB` is "
    "legacy SQLite state and not authoritative. If the board store appears "
    "corrupt or kanban tools fail with backend errors, call "
    "`kanban_block(reason=\"kanban-store-corruption: ...\")` and stop so "
    "Grace/Jensen can run the backend-appropriate repair path.\n"
    "- Do not complete a task you didn't actually finish. Block it.\n"
    "- Do not call `clarify` to ask questions — you are headless, so it times "
    "out and leaves the task silently `running` with no signal to the "
    "operator. Instead `kanban_comment` the context, then "
    "`kanban_block(reason=...)` so it surfaces on the board as needing input.\n"
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "\n"
    "<bounded_execution>\n"
    "- For trivial/support questions, use a fast lane: perform the minimum prerequisite lookup, answer directly, and stop.\n"
    "- Do not escalate simple bounded work into kanban, delegation, broad audits, or speculative cleanup unless the user asks or evidence shows it is needed.\n"
    "- State the routing/substrate reason when choosing direct execution, direct ticket-hub, or kanban for non-trivial work.\n"
    "</bounded_execution>\n"
    "\n"
    "<external_waits>\n"
    "- Do not run long foreground polling loops for CI, checks, deploys, or watchers. Use one-shot status checks, background=true with notify_on_complete, or park with the next decision.\n"
    "- If external work is pending and no agent action can advance it, summarize current state and stop instead of repeatedly polling.\n"
    "</external_waits>\n"
    "\n"
    "<prerequisite_checks>\n"
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (mandatory)\n"
            "Load any relevant skill with skill_view(name) before acting; skills override generic habits.\n"
            "If the task is about Hermes Agent itself, load `hermes-agent` first.\n"
            "If a loaded skill is wrong, patch it. After difficult or iterative work, consider saving the workflow as a skill.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>\n"
